...
class  ExportFile
{
	...
	public static function paymentListPdf($options=false)
	{
		$db=Database::connect();
		$request=Services::request();
		$reportModel=new ReportModel();
		...
		$month=checkVariable($get['month'],0,'intval');
		$search=checkVariable($get['search'],'','trim');
		$fromDate=checkVariable($get['fromDate'],'','trim');
		$toDate=checkVariable($get['toDate'],'','trim');
		$sortBy=checkVariable($get['sortBy'],0,'intval');
        $parameters=array('type'=>0);
		$userSelectedFields=checkVariable($get['fieldNames'],0);
		$websiteDetails = new \Config\WebsiteDetails();
		$projectName=checkVariable($websiteDetails->projectName,'','trim');
		$fileTitle=' Payment List';
		...
		array('name'=>'Date','field'=>'labDate','width'=>20,'position'=>2),
		array('name'=>'Patient Name','field'=>'patientName','width'=>40,'position'=>3),
		array('name'=>'Service Name','field'=>'services','width'=>70,'position'=>4),
		array('name'=>'Amount','field'=>'amount','width'=>30,'position'=>5),
		);
		$userSelectedFields=array(0,1,2,3,4);
		$paymentModel = new PaymentModel();
		$totalRecords=$paymentModel->getList($parameters);
		if($totalRecords>0)
		{
		$parameters['action']=1;
		$parameters['isMultiple']=1;
		$parameters['limit']=$totalRecords;
		$parameters['type']=1;
		$parameters['fieldSeparator']=',';
		$result=$paymentModel->getList($parameters);		
		$maximum=0;
		if(isEmptyArray($result)>0)
		{
		$showFields=array();
		...
		*
		...
		$key=$key+1;
		$patientName=checkVariable($row['patientName'],0,'trim');
		$services=checkVariable($row['services'],0,'trim');
		$amount=checkVariable($row['totalAmount'],0,'doubleval');
		$doctorID=checkVariable($row['doctorID'],0,'intval');
		$labDate=checkVariable($row['labDate'],0);
		$labDate=(!empty($labDate)) ? date("d-m-Y",strtotime($labDate)) : '';
		...
		$data.='<tr><td class="text-right" colspan="'.(isEmptyArray($showFields)-2).'"><p> Total Amount:</p></td><td colspan="2"><p><b>&#8377; '.convertIntoIndianRupesh($totalAmount).'</b></p></td></tr>';
		$data.='</tbody>
		</table>
		</body>
		</html>
	...
	public static function paymentListExcel($options=false)
	{
		$db=Database::connect();
		$request=Services::request();
		$reportModel=new ReportModel();
		...
		$sheet->getCell('A'.$i)->setValue($totalAmount);
		$styleArray = [
		'font' => [
		'bold' => false,
		'size'=>12
